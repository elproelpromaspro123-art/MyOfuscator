...
const features = [
  ...
  { name: 'XOR Encryption', levels: [2, 3, 4] },
  { name: 'Environment Check', levels: [2, 3, 4] },
  { name: 'Opaque Predicates', levels: [2, 3, 4] },
  { name: 'Multi-Key Rolling XOR', levels: [3, 4] },
  { name: 'Anti-Tamper Check', levels: [3, 4] },
  { name: 'Control Flow Obfuscation', levels: [3, 4] },
  { name: 'String Table Shuffle', levels: [4] },
  { name: 'Triple XOR Encryption', levels: [4] },
]
...
export default function SettingsPanel({ settings, onChange }: Props) {
  ...
  return (
    <div className="card p-6 space-y-6">
      ...
      <div>
        ...
        <div className="flex items-center gap-3 p-3 rounded-lg bg-[#151519] border border-[#1f1f28]">
          ...
          <div>
            ...
            <div className="text-xs text-zinc-500">Executor Compatible</div>
          </div>
      ...
      {/* Features List */}
      <div>
        <h3 className="text-xs font-medium text-zinc-500 uppercase tracking-wider mb-3">Active Features</h3>
          ...
          {features.map(f => {
            ...
            return (
              ...
              >
                ...
                <span className={`text-sm ${isActive ? 'text-white' : 'text-zinc-600'}`}>
                  {f.name}
                </span>
              </div>
            )
      ...
      <div className="pt-4 border-t border-[#1f1f28] space-y-2">
        <div className="flex items-center gap-2 text-xs text-green-500">
          ...
          <span>All strings hidden (URLs, text, etc.)</span>
        </div>
        <div className="flex items-center gap-2 text-xs text-green-500">
          <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13 10V3L4 14h7v7l9-11h-7z" />
          </svg>
          <span>Executor compatible (IIFE wrapper)</span>
        </div>
        <div className="flex items-center gap-2 text-xs text-green-500">
          <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z" />
          </svg>
          <span>No loadstring in output</span>
        </div>
      </div>
    </div>
  )
