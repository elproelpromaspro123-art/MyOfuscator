    ...
    <div className="space-y-3 fade-in">
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        ...
        <div className="card p-4 text-center group hover:border-[#6366f1]/50 transition-all">
          ...
          <div className="text-2xl font-bold gradient-text">{stats.stepsApplied.length}</div>
        </div>
      </div>
      ...
      <div className="card p-4">
        <div className="flex items-center gap-2 mb-3">
          ...
          <span className="text-xs text-zinc-500 uppercase tracking-wider font-medium">Steps Applied</span>
        </div>
        <div className="flex flex-wrap gap-2">
          {stats.stepsApplied.map((step, i) => (
            <span 
              key={i} 
              className="px-2 py-1 text-xs rounded-md bg-[#151519] border border-[#1f1f28] text-zinc-400"
            >
              {step}
            </span>
          ))}
